        ...
        protected override void Start()
        {
            ...
            for (int i = 0; i < persistentGroundStations.Count; i++)
            {
                if (!groundStations.Exists(x => x.ID.Equals(persistentGroundStations[i].ID))) //need to create ground stations
                {
                    ...
                    var additionalStation = UnityEngine.Object.Instantiate<RemoteTechCommNetHome>(stockStation); //working
                    ...
                    Logging.Info("Custom CommNet Home '{0}' added", persistentGroundStations[i].nodeName);
                }
